...
tehtavaController.get("/", async (req, res) => {
  const tehtavat = await Tehtava.find({}).populate("kayttaja", { nimi: 1 });
  res.json(tehtavat);
});
...
tehtavaController.put("/:id", async (req, res) => {
  ...
  /* const muokattu = await Tehtava.updateOne({_id: req.params.id}, { tehtava }) */
  if (muokattu) {
...
tehtavaController.delete("/:id", async (req, res, next) => {
  ...
  kayttajaTehtavat = kayttajanTehtavat.map((n) => {
    if (n != req.params.id) {
  ...
  const muokattu = await Kayttaja.findByIdAndUpdate(
    decodedToken.id,
    uusiKayttaja
  );
});
